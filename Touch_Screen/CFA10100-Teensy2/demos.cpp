...
uint16_t Add_Bounce_To_Display_List(uint16_t FWol)
  ...
  // Make it transparent
  FWol=EVE_Cmd_Dat_0(FWol, EVE_ENC_COLOR_A(transparency));

  // Draw the ball -- a point (filled circle)
  FWol=EVE_Point(FWol, x_position,y_position,ball_size);
  ...
  FWol=EVE_Cmd_Dat_0(FWol,EVE_ENC_COLOR_RGB(200,0,0));
  //(transparency set above still in effect)
  FWol=EVE_Line(FWol,LCD_WIDTH/2,LCD_HEIGHT/2,x_position/16,y_position/16,rubberband_width);
  return(FWol);
  }
//---------------------------------------------------------------------------
